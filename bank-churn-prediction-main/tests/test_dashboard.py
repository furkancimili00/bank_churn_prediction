...
def test_make_prediction_eksik_ozellikler():
    ...
    sonuc = dashboard.make_prediction(eksik_musteri_verisi)
    ...
    cagri_argumanlari = dashboard.local_scaler.transform.call_args
    ...
    eksik_olan_sutunlar = [col for col in mock_features if col not in ["CreditScore", "Age", "Balance"]]
    for col in eksik_olan_sutunlar:
        assert df_scaler_input.iloc[0][col] == 0, f"Sütun {col} 0 değil, değeri: {df_scaler_input.iloc[0][col]}"
