...
if 'audit_no_logs' not in i18n_content:
    updated_i18n = tr_part + new_tr_str + "\\n" + en_part[:idx_en_end] + new_en_str + "\\n" + en_part[idx_en_end:]
...
for fp in glob.glob('ui/tabs/*.py'):
    ...
    # Only add import if we actually replace something
    will_replace = any(tr_val in code for tr_val in MAPPING.keys())
    ...
    if will_replace:
        ...
        for tr_val, t_call in MAPPING.items():
            # literal matching
            # try double quotes
            code = code.replace(f'"{tr_val}"', t_call)
            # try single quotes
            code = code.replace(f"'{tr_val}'", t_call)
