...
new_funcs = '''def _render_ai_campaign_messages(agent_result: dict[str, Any]) -> None:
    ...
    def create_mailto_url(body_text):
        if not body_text:
            return ""
        # Mailto limitlerine takılmamak ve yeni satırları korumak için:
        # \n leri %0D%0A yaparız.
        body_text = body_text.replace('\\n', '\r\n')
        # Sadece Outlook vs. için body'i encode et
        safe_body = urllib.parse.quote(body_text)
        return f"mailto:?subject=Size%20Özel%20Kampanya%20Fırsatı&body={safe_body}"

    if gemini_msg or groq_msg or openai_msg:
        st.markdown("### 🤖 Otonom Ajan Kampanya Üretimleri", help="Yapay zeka modellerinin ürettiği kampanya senaryoları")

        if gemini_msg:
            with st.container(border=True):
                st.markdown("#### 🔵 Google Gemini Senaryosu")
                st.success(gemini_msg)
                st.link_button("✉️ E-Posta Şablonunu Aç (Gemini)", create_mailto_url(gemini_msg))

        if groq_msg:
            with st.container(border=True):
                st.markdown("#### 🟠 Groq Llama3 Senaryosu")
                st.info(groq_msg)
                st.link_button("✉️ E-Posta Şablonunu Aç (Groq)", create_mailto_url(groq_msg))

        if openai_msg:
            with st.container(border=True):
                st.markdown("#### 🟢 OpenAI GPT Senaryosu")
                st.error(openai_msg)
                st.link_button("✉️ E-Posta Şablonunu Aç (OpenAI)", create_mailto_url(openai_msg))
        return
    ...
    with st.container(border=True):
        ...
        if campaign_msg:
            ...
            st.link_button("✉️ E-Posta Şablonunu Aç", create_mailto_url(campaign_msg))
        else:
            st.warning("Henüz mesaj oluşturulmadı.")
